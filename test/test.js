...
test("Performing diffs on " + modifiedJSON.length + " autogenerated JSON samples produces expected results. See console for stats.", modifiedJSON.length, function() {
    var diff_lengths = [];

    for (var ii = 0; ii < modifiedJSON.length; ii++) {
        diff = jsondiffpatch.diff(originalJSON, modifiedJSON[ii]);
        diff_lengths.push(JSON.stringify(diff).length);

        expected_output = modifiedJSON[ii];
        actual_output = jsondiffpatch.patch(originalJSON, diff);
        deepEqual(actual_output, expected_output, ' sample JSON #' + ii + ' did not pass.');
    }
    var diff_properties = {
        'Average Diff Length': getAverageFromNumArr(diff_lengths, 2),
        'Diff Length Standard Deviation': getStandardDeviation(diff_lengths, 2)
    };
    console.log('Created and applied ' + modifiedJSON.length + ' diffs:')
    console.log(diff_properties);
});
