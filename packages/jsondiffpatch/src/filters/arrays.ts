...
export const diffFilter: Filter<DiffContext> = function arraysDiffFilter(
	...
	let child: undefined | DiffContext;
	...
	for (index = commonHead; index < len2 - commonTail; index++) {
		...
		if (indexOnArray2 < 0) {
			...
			if (seq.indices1[indexOnArray2] === undefined) {
				throw new Error(
					`Invalid indexOnArray2: ${indexOnArray2}, seq.indices1: ${seq.indices1}`,
				);
			}
			index1 = seq.indices1[indexOnArray2] + commonHead;
			if (seq.indices2[indexOnArray2] === undefined) {
				throw new Error(
					`Invalid indexOnArray2: ${indexOnArray2}, seq.indices2: ${seq.indices2}`,
				);
			}
	...
	for (index in delta) {
		if (index !== "_t") {
			if (index[0] === "_") {
				...
				if (
					delta[removedOrMovedIndex] !== undefined &&
...
export const reverseFilter: Filter<ReverseContext> =
	function arraysReverseFilter(context) {
			...
			if (nonNestedDelta[2] === ARRAY_MOVE) {
				...
				context
					.setResult([
						arrayMoveDelta[0],
						Number.parseInt(
							(context.childName as `_${number}`).substring(1),
							10,
						),
						ARRAY_MOVE,
					])
		...
		for (const name in arrayDelta) {
			...
			const child = new ReverseContext(arrayDelta[name as `${number}`]);
