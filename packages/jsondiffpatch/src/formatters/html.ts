...
class HtmlFormatter extends BaseFormatter<HtmlFormatterContext> {
	...
	nodeBegin(
		...
		const label =
			typeof leftKey === "number" && key.substring(0, 1) === "_"
				? key.substring(1)
				: key;
		context.out(
			`<li class="${nodeClass}" data-key="${htmlEscape(key)}">` +
...
const adjustArrows = function jsondiffpatchHtmlFormatterAdjustArrows(
	...
	const getElementText = ({ textContent, innerText }: HTMLElement) =>
	...
	eachByQuery(
		...
		({ parentNode, children, style }) => {
			...
			const moveDestinationElem = arrowParent.querySelector(
