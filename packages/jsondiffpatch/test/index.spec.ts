...
import { assertNonEmptyArray } from "../src/assertions/arrays.js";
...
describe("DiffPatcher", () => {
	for (const groupName in examples) {
		const group = examples[groupName];
		if (!group) throw new Error("test group undefined");
		describe(groupName, () => {
	...
	describe("using cloneDiffValues", () => {
		...
		it("ensures deltas don't reference original array items", () => {
			const left = {
				list: [
					{
						id: 1,
	...
	describe("using omitRemovedValues", () => {
		...
		it("ensures deltas don't have the removed values", () => {
			const left = {
				oldProp: {
